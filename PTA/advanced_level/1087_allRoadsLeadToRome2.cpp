...
int e[205][205], weight[205], dis[205];     // weight: happiness
bool visit[205];
...
void dfs(int v) {
    ...
    if(v == 1) {
        ...
        double tempavg = 1.0 * value / (temppath.size() - 1);   // 这边1.0*的操作看起来很妙的样子！但是有什么用呢？
        if(value > maxvalue) {
            maxvalue = value;
            maxavg = tempavg;
            path = temppath;        // 拷贝构造
    ...
    temppath.pop_back();    // 回溯法，把下探到的前一个删除
}
int main() {
