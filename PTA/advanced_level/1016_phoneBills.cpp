...
int lastingT(string time1, string time2) {      // 这一步可以通过计算累计时间优化
...
double cost(string time1, string time2) {       // 这一步也可以通过计算累计cost优化
    double result = 0.0;
    int d1, d2, h1, h2, m1, m2;
    d1 = atoi(time1.substr(3, 2).c_str());
    h1 = atoi(time1.substr(6, 2).c_str());
...
bool compName(vector<Record*> v1, vector<Record*> v2) {     // 这一步....不想吐槽了，string又内置函数比较字典顺序的...
...
bool compTime(Record* r1, Record* r2) {         // 如果计算累计时间，这一步直接就可以被省略
